...
int main(int argc, char** argv) {
  ...
  while(fgets(sline,sizeof(sline),fpdst))
  {
    ...
    const char* img_path = (DATA_DIR + sline).c_str();
    ...
    std::cout<<"the image path is " << (DATA_DIR + sline).c_str();
    cv::Mat img = cv::imread((DATA_DIR + sline).c_str(), cv::IMREAD_UNCHANGED);
    cv::Mat img_gray;
    ...
    //cv::imshow("Test", img);
    cv::imwrite((DATA_OUTPUT_DIR+sline).c_str(), img);
    //cv::waitKey(0);
