...
std::string DATA_OUTPUT_DIR = "../../data/output/";
...
int main(int argc, char** argv)
{
  ...
  //argv[1] is a filelist
  FILE *fpdst = fopen(argv[1],"r");
  if(NULL == fpdst)
  {
  ...
  char sline[1024];
  int num = 0;
  while(fgets(sline,sizeof(sline),fpdst))
  {
    if(NULL == sline)
      break;
    ...
    img_grayscale = cvLoadImage((DATA_DIR + sline).c_str(), 0);
    if (img_grayscale == NULL)
    {
      std::cout<< "this file is empty "<<sline<<std::endl;
      continue;
    }

    IplImage *img_color = cvLoadImage((DATA_DIR + sline).c_str(), 1);
    int pts_num = 5;
    ...
    // Detect faces
    std::cout<<"the faces is " << image_data.width<<" "<< image_data.height << " "<< image_data.num_channels << std::endl;
    ...
    if (face_num == 0)
    {
      ...
      std::cout<<"this file has no face" << sline<<std::endl;
      //return 0;
      continue;
    }
    ...
    cvSaveImage((DATA_OUTPUT_DIR + sline).c_str(), img_color);
