...
void TEST(FaceRecognizerTest, CropFace) {
  ...
  ifs.open(test_dir + "test_file_list1.txt", std::ifstream::in);
  clock_t start, count = 0;
  int img_num = 0;

  getline(ifs,img_name);
  while (ifs.good()) {
    std::cout << ifs.good();
    std::cout << "img_name "<<img_name<<endl;
    img_num ++ ;
    // read image
    cv::Mat src_img = cv::imread(test_dir + img_name, 1);
    std::cout << "load image " << img_name.c_str()<< std::endl;
    ...
    std::string test_file_dir = img_num + ".jpg";
    //std::cout << "save new crop image "<< img_name << " "<< test_file_dir << std::endl;
    dst_img.data = dst_img_data.data;
    cv::imwrite("1.jpg", dst_img);
    // Show crop face
    //    cv::imshow("Crop Face", dst_img);
    //    cv::waitKey(0);
    //    cv::destroyWindow("Crop Face");
    getline(ifs,img_name);
    std::cout << "img_name1 "<<img_name<<endl;
  }
  std::cout<< ifs.good();
  ifs.close();
...
void TEST(FaceRecognizerTest, ExtractFeatureWithCrop) {
  ...
  /* Data initialize */
  std::ifstream ifs(test_dir + "test_file_list1.txt");
  std::string img_name;
  FacialLandmark pt5[5];
  ...
  while (ifs >> img_name) {
    ...
    for (int i = 0; i < 5; ++ i) {
      ifs >> pt5[i].x >> pt5[i].y;
      std::cout<<pt5[i].x << pt5[i].y<<std::endl;
    }
